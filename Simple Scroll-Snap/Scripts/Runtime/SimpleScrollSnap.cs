...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        {
            Initialize();
            startingPanel =  PlayerPrefs.GetInt("LevelUnlocked",1)-1;
        }
        private void Start()
        {
            if (Validate())
        ...
        private void Update()
        {
            ...
            {
                if(PlayerPrefs.GetInt("SaveCurrentIndex",0)==1)
                {
                    PlayerPrefs.SetInt("SaveCurrentIndex",0);
                    GoToPanel(PlayerPrefs.GetInt("CurrentLevel",1)-1);
                }
                OnOcclusionCulling();
